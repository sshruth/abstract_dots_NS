function [save_struct, coh_frame, scr_info] = regen_dots(eMat)
...
for i=1:ntr
    ...
    duration                    = (eMat(i,E.time_dots_off)-eMat(i,E.time_dots_on)); 
    ...
    dots_struct.dot_size        = 3;
    dots_struct.dot_color       = [255 255 255];
    
    dots_struct.dot_pos         = [];
    dots_struct.shown_frames    = 0;
    
    dots_struct.seed            = rseed;
    ...
    if ~isnan(duration)
        [dots_struct,coh_frame{i}] = dotsRemake(rseed,duration,dots_struct,screen_struct,MAX_DOTS_PER_FRAME,...
            screen_width_cm,distance_to_screen_cm);
    else
        dotpos{i} = [];
        coh_frame{i} = [];
    end
    save_struct(i).dots_struct = dots_struct;
end
