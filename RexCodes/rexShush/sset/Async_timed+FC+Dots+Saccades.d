...
int							  gl_prize_count;
int							  gl_prize_max = 0;
int 							gl_rt_flag = 0; /*current trial type */
...
int               gl_rew_fc_prob = 1000;    /* free choice reward probability */
int               gl_rew_fc = 0;    /* free choice reward given on a trial */
int 							gl_rew_fix_prob = 0; /*probability of being rewarded for fixation*/
int 							gl_rew_fix = 0; /* whether to give fixation reward on a trial */
int               gl_async_ctr = 0; /* asynchrony frame counter */
int               gl_async_cur = 0; /* current asynchrony */
int               gl_fixed_win = 0; /* set fixed target windows*/
...
int 		gl_sacctask = 0; /* flag to branch to saccade task */
int 		gl_saccdelay = 0; /* overlap or memory saccade */
int 		gl_nsacctrls; /* no. of saccade trials to do */
int 		gl_sacctrg_rnd; /* flag to randomize targets */
int 		gl_saccrfr;
int 		gl_saccrft;
int 		gl_sacctrlctr = 0;
int 		gl_sacctrlremain = 0;
int 		*gl_sacc_xpos = NULL;
int		 	*gl_sacc_ypos = NULL;
int 		*gl_sacc_trlidx = NULL;
...
/*  ROUTINE: defTargBlkWhite
**  Sets the sequence of target appearances for async task
*/
int defTargBlkWhite()
{
	RGB color;
  _VSobject obj_v;
  int tar1val, tar2val, angl, lateLum, coh;
  int asyncf[6] = {234, 154, 100, 47, 34, 20}; // 6 ms less than frames {18, 12, 8, 4, 3, 2} @75Hz;
	int async_max = 245; // Set to frame one more than highest async used {19}
	_VSDdot_object dot = VSD_GET_DTOBJECT(gl_vsd);
	coh = dot->coherence;
	angl = dot->direction;

		if (coh==756){
		    gl_async_cur = asyncf[0];
        async_max = 221; // Need to drop two frames as a 0 timer runs in this condition
		}else if (coh==512){
		    gl_async_cur = asyncf[1];
		}else if (coh==256){
		    gl_async_cur = asyncf[2];
		}else if (coh==128){
		    gl_async_cur = asyncf[3];
		}else if (coh==64){
		    gl_async_cur = asyncf[4];
		}else if (coh==32){
		    gl_async_cur = asyncf[5];
		}else {
		    gl_async_cur = 0;
        async_max = 221; // Need to drop two frames as 0 is treated differently
		}

		gl_async_ctr = async_max - gl_async_cur; /* To keep first target on for same time */

		EC_TAG2(I_SEEDCD, gl_async_cur);
		printf("\n");
		printf("Async frames = %d (coh = %d)\n", gl_async_cur,coh);
		printf("Direction = %d\n", angl);
		printf("\n");

		if (gl_async_cur==0) /*To make sure that both targets come on at the same time in 0 async*/
		lateLum = 1000;
		else lateLum = 0;
		...
		if (angl==0){
		    tar1val = lateLum;
		    tar2val = 1000;
		    EC_TAG2(I_TRG1LCD, lateLum/1000);
		    EC_TAG2(I_TRG2LCD, 1);
		} else{
		    tar1val = 1000;
		    tar2val = lateLum;
		    EC_TAG2(I_TRG1LCD, 1);
		    EC_TAG2(I_TRG2LCD, lateLum/1000);
		}
		defTargLum(-1,tar1val,tar2val,-1);

		return 0;
}

/*ROUTINE: set_async_timer
**
** Allows timers based on async value of trial
*/
int set_async_timer(long who_calling)
 {
	long duration;

	if (who_calling==1) /* async interval */
		gl_rtvar.delay = timer_set1(0,0,0,0,gl_async_cur,0);
	else /* to achieve constant first target time */
		gl_rtvar.delay = timer_set1(0,0,0,0,gl_async_ctr,0);
...
/* ROUTINE setup_eyewindow
 **
 ** sets up both target window locations at once
 */
...
/* ROUTINE setup_hemiwindows
 **
 ** sets up target window locations at fixed locations in
 ** left and right hemifields
 ** useful for fixed windows when targets are jittering
 */

int setup_hemiwindows(long wd_x, long wd_y, long wd_width, long wd_height)
{
    _VSobject vso;
    ...
    // wd_pos(WIND3, vso->x, vso->y);
    wd_pos(WIND3, wd_x*gl_correct_side, wd_y);
    wd_siz(WIND3, wd_width, wd_height);
    wd_cntrl(WIND3, WD_ON);

    vso = VSD_GET_TW(gl_vsd);
    //wd_pos(WIND4, vso->x, vso->y);
    wd_pos(WIND4, wd_x*gl_correct_side*-1, wd_y);
    wd_siz(WIND4, wd_width, wd_height);
    wd_cntrl(WIND4, WD_ON);
...
/* ROUTINE: set_fix_rwd
** give reward on a subset of trials for just acquiring fixation
*/
int set_fix_rwd(void)
{
	if (TOY_RAND(1000)<gl_rew_fix_prob){
		gl_rew_fix = 1;
	}else{
		gl_rew_fix = 0;
	}
...
*/
int give_prize(long dio, long duration, long who_calling)
{
 ...
 /* ROUTINE: prob_reward
 **
 ** description: probabilistic reward for free choice tasks
 */
int prob_reward()
{
    if (TOY_RAND(1000)<gl_rew_fc_prob)
    gl_rew_fc = 1;
    else gl_rew_fc = 0;

    printf("FC trial reward = %d\n",gl_rew_fc);
    return 0;
}
...
** prizes are additional rewards given to the monkey for doing several trials correct in a row
** <gl_prize_count> is a global variable that is updated by  <update_prize_count>
** at the end of each trial.
** maximum number of prizes will be managed by <set_max_prize_count>
**
*/
...
int dropcode_sbet(void)
 ...
 /* SACC TASK ROUTINES
 */
 int makeSaccTask(void) {
 	int i, cx1, cx2, cy1, cy2;
	printf("Randomizing saccade locations\n");
 	/* x positions */
 	gl_sacc_xpos = (int *) realloc(gl_sacc_xpos,gl_nsacctrls * sizeof(int));
 	if (gl_sacc_xpos == NULL) {
 		free(gl_sacc_xpos);
 		gl_sacc_xpos = NULL;
 		printf("Could not allocate memory for gl_sacc_xpos\n");
 	}

 	/* y positions */
 	gl_sacc_ypos = (int *) realloc(gl_sacc_ypos,gl_nsacctrls * sizeof(int));
 	if (gl_sacc_ypos == NULL) {
 		free(gl_sacc_ypos);
 		gl_sacc_ypos = NULL;
 		printf("Could not allocate memory for gl_sacc_ypos\n");
 	}

 	/* trial indices */
 	gl_sacc_trlidx = (int *) realloc(gl_sacc_trlidx,gl_nsacctrls * sizeof(int));
 	if (gl_sacc_trlidx == NULL) {
 		free(gl_sacc_trlidx);
 		gl_sacc_trlidx = NULL;
 		printf("Could not allocate memory for gl_sacc_trlidx\n");
 	}

	 /* Fill in the two target locations from record */
	 cx1 = (VSD_GET_T1(gl_vsd))->x;
	 cy1 = (VSD_GET_T1(gl_vsd))->y;
	 cx2 = (VSD_GET_T2(gl_vsd))->x;
	 cy2 = (VSD_GET_T2(gl_vsd))->y;

 		float x[]={cx1, cx2};
 		float y[]={cy1, cy2};

   	for (i = 0; i<gl_nsacctrls; i++) {
       //printf("here %f\n", x[i%7]);
       gl_sacc_xpos[i] = x[i%2];
       gl_sacc_ypos[i] = y[i%2];
 		}

 	/* shuffle up the trials */
 	for (i = 0; i < gl_nsacctrls; i++)
 	  gl_sacc_trlidx[i]=i;
 	for (i = gl_nsacctrls-1; i > 0; i--) {
 		int c = rand() % gl_nsacctrls;
 		if (c > gl_nsacctrls-1)
 		  printf("uh oh");
 		int t = gl_sacc_trlidx[c];
 		gl_sacc_trlidx[c] = gl_sacc_trlidx[i];
 		gl_sacc_trlidx[i] = t;
 	}

 	gl_sacctrlctr = gl_nsacctrls-1;
 	printf("\nX positions = \n");
 	for ( i = 0; i < gl_nsacctrls; i++) {
 		printf("%d  ", gl_sacc_xpos[gl_sacc_trlidx[i]]);
 	}
 	printf("\n");
 	printf("\nY positions = \n");
 	for ( i = 0; i < gl_nsacctrls; i++) {
 		printf("%d  ", gl_sacc_ypos[gl_sacc_trlidx[i]]);
 	}
 	printf("\n");
 	if (gl_sacctrlctr >= 0)
 	  gl_sacctrlremain = 1;
 ...
 /* SACC ROUTINE: saccSetup
  ** Set up the current trial parameters and send it to PTB and plexon
  */

 int saccSetup()
 {
	if (gl_sacctrg_rnd) { /* draw location from randomized array */
 		if (gl_sacctrlctr < 0) {
 			gl_sacctrlremain = 0;
 			return(0);
 		} else {
 			gl_targObj.x = (int) round(gl_sacc_xpos[gl_sacc_trlidx[gl_sacctrlctr]]);
 			gl_targObj.y = (int) round(gl_sacc_ypos[gl_sacc_trlidx[gl_sacctrlctr]]);
 			gl_sacctrlremain = 1;
			gl_sacctrlctr--;
 		}
 	} else { /* present the single location specified */
		// gl_targObj.x = (VSD_GET_T1(gl_vsd))->x;
	 	// gl_targObj.y = (VSD_GET_T1(gl_vsd))->y;

 		gl_targObj.x = TOY_RT_TO_X(0,gl_saccrfr,gl_saccrft);
    gl_targObj.y = TOY_RT_TO_Y(0,gl_saccrfr,gl_saccrft);
		gl_sacctrlremain = 1;
 	}
	printf("\nCurrent target: X=%d, Y=%d \n", gl_targObj.x, gl_targObj.y);
	printf("Trials remaining: %d \n", gl_sacctrlctr);
	...
	/* send task relevant codes */
 	EC_TAG2(I_TRIALIDCD,gl_saccdelay+10); // 10 is overlap, 11 is memory guided
	...
	// EC_TAG2(I_TRG1LCDR, gl_targObj.color.R); // sending target RGB info
	// EC_TAG2(I_TRG1LCDG, gl_targObj.color.G);
	// EC_TAG2(I_TRG1LCDB, gl_targObj.color.B);
...
/* SACC ROUTINE: drawSaccTarg
 ** 1 to show, 0 to erase
 */
int drawSaccTarg(long showFixEye, long showTarg)
{
	int num_show=0, num_hide=0;
	int show_ind[2], hide_ind[2];
...
/* SACC ROUTINE: setSaccTarWindow
** Window size and location for saccade target
 */
int setSaccTarWindow(long wd_width, long wd_height)
{
	wd_pos(WIND3, gl_targObj.x, gl_targObj.y);
	wd_siz(WIND3, wd_width, wd_height);
	wd_cntrl(WIND3, WD_ON);
	EC_TAG2(I_ETARG_ACCEPTHX, wd_width);
	EC_TAG2(I_ETARG_ACCEPTHY, wd_height);
...
//	}
    ...
    if (gl_rt_flag >= 0){ /* For variable duration (0), RT (1), and free choice (2). Note that this change may screw up the functionality to pick target locations based on dot direction/angle. */
        ...
        if (TOY_RAND(1000) < 500){
        	i = gl_menu_infoS.rfr + TOY_RAND(gl_menu_infoS.rfr_jit);
        } else {
        	i = gl_menu_infoS.rfr - TOY_RAND(gl_menu_infoS.rfr_jit);
        }
       if (TOY_RAND(1000) < 500){
        	j = angl + TOY_RAND(gl_menu_infoS.rft_jit);
        } else {
        	j = angl - TOY_RAND(gl_menu_infoS.rft_jit);
        }
        gl_vsd->display->object_array[VSD_OBJECT_T1]->x = TOY_RT_TO_X(0,i,j);
        gl_vsd->display->object_array[VSD_OBJECT_T1]->y = TOY_RT_TO_Y(0,i,j);
       ...
       if (TOY_RAND(1000) < 500){
        ...
        gl_vsd->display->object_array[VSD_OBJECT_T2]->x = -1* TOY_RT_TO_X(0,i,j); /*Flip around vertical axis*/
        gl_vsd->display->object_array[VSD_OBJECT_T2]->y = TOY_RT_TO_Y(0,i,j);
    }
	...
	/* save the parameters */
	/* task identifier */
    EC_TAG2(I_TRIALIDCD,40+gl_rt_flag); // 40 is variable duration dots, 41 is RT, 42 is freechoice, 43 is async target
	EC_TAG2(I_MONITORDISTCD, VIEW_DIST_CM);
	/* fixation x, y, diameter */
	EC_TAG1(I_FIXXCD, (VSD_GET_FP(gl_vsd))->x);
	EC_TAG1(I_FIXYCD, (VSD_GET_FP(gl_vsd))->y);
...
int total(long score)
{
	...
	/* set react time variable for RT to zero, revise below if an RT task */
	gl_rtvar.rt = 0;
...
VLIST state_vl[] = {
    ...
    {"RF_R_jitter",             &(gl_menu_infoS.rfr_jit),		    NP, NP, 0, ME_DEC},
    {"RF_T_jitter",             &(gl_menu_infoS.rft_jit),		    NP, NP, 0, ME_DEC},
    {"RNG_Seed",			    			&(gl_menu_infoS.seed),		      NP, NP, 0, ME_DEC},
		{"FC_reward_P",             &gl_rew_fc_prob,                NP, NP, 0, ME_DEC},
		{"Fix_reward_P",            &gl_rew_fix_prob,               NP, NP, 0, ME_DEC},
    {"Set_targ_window",         &gl_fixed_win,                  NP, NP, 0, ME_DEC},
    {"Max_prize_cnt",           &gl_prize_max, 									NP, NP, 0, ME_DEC},
		{"Proportn_TEYE",           &(gl_task_infoS[0].proportion), NP, NP, 0, ME_DEC},
    {"Joystick",								&(gl_menu_infoS.joystick), 			NP, NP, 0, ME_NVALD},
    {NS}};

	VLIST saccpars_vl[] = {
    {"Interleave_sacc",         &gl_sacctask, 									NP, NP, 0, ME_DEC},
		{"Memory",             			&gl_saccdelay,                	NP, NP, 0, ME_DEC},
		{"Randomize_tars",          &gl_sacctrg_rnd,                NP, makeSaccTask, ME_AFT, ME_DEC},
		{"Num_trials",             	&gl_nsacctrls,                	NP, NP, 0, ME_DEC},
		{"Targ_radius",             &gl_saccrfr,                		NP, NP, 0, ME_DEC},
		{"Targ_theta",             	&gl_saccrft,                		NP, NP, 0, ME_DEC},
		{"Targ_size",            		&(gl_targObj.diameter),         NP, NP, 0, ME_DEC},
    {"Targ_color_R",         	 	&(gl_targObj.color.R),          NP, NP, 0, ME_DEC},
		{"Targ_color_G",         	 	&(gl_targObj.color.G),          NP, NP, 0, ME_DEC},
		{"Targ_color_B",         	 	&(gl_targObj.color.B),          NP, NP, 0, ME_DEC},
    {NS}};
...
MENU umenus[] = {
   {"State_vars", 		&state_vl, 			NP, NP, 0, NP, no_help},
	 {"Saccade_pars", 	&saccpars_vl, 	NP, NP, 0, NP, no_help},
   {"separator", 		NP},
...
begin	first:
	...
	branchtask: /* BRANCH between saccade or regular task ****/
		to sacc_start on 1 = gl_sacctask
		to settrl
	settrl:		/* set up current trial for non-saccade tasks */
		do nexttrl(0, 10, 1)
		to task on +ONES & gl_remain
		to loop
	task:
    to teye_fixed on 1 = gl_fixed_win
    to teye_start

	/* SACCADE TASKS BRANCH
	*/
	sacc_start:
		time 250
		to sacc_setup on 0 = gl_sacctrg_rnd /* no saccade target randomization */
		to sacc_loop
	sacc_loop:
		time 250
		to sacc_mktrls on 0 = gl_sacctrlremain
		to sacc_setup
	sacc_mktrls:
		do makeSaccTask()
		to sacc_setup on +ONES & gl_sacctrlremain
		to sacc_start
	sacc_setup:
		do saccSetup()
		to sacc_fixwin
	sacc_fixwin:
		do position_eyewindow(20, 20, 0)
		time 10
		to sacc_tarwin
	sacc_tarwin:
		do setSaccTarWindow(50, 50)
		time 10
		to sacc_drawfp
	sacc_drawfp:
		do drawTarg(1000,0,0,0,0)
		to sacc_fp_cd on MAT_WENT % drawTarg_done
	sacc_fp_cd:
		do ec_send_code(FPONCD)
		to sacc_fixeyewait
	sacc_fixeyewait:
		time 5000
		to sacc_fixeyedelay on -WD0_XY & eyeflag
		to sacc_nofix
	sacc_nofix:
		time 2000
		...
		to loop
  sacc_fixeyedelay:
		time 100 /* delay for stabilizing eye signal before activating eye_flag */
		to sacc_dropfixcode
	sacc_dropfixcode:
 	 	do set_eye_flag(E_FIX)
		to sacc_fixeyedone
  sacc_fixeyedone:		/* change fix window size if needed */
 		do position_eyewindow(25, 25, 0)
 	 	to sacc_targ_wait
	sacc_targ_wait:
		do timer_set1_shell(1000,200,600,100,0,0)
		to sacc_drawtarg on +MET % timer_check1
	sacc_drawtarg:
		do drawSaccTarg(1, 1)
		to sacc_targ_cd on MAT_WENT % drawTarg_done
	sacc_targ_cd:
		do ec_send_code(TARGC1CD)
		to sacc_removeTarWait on 1 = gl_saccdelay /* branch to memory */
		to sacc_waitfpoff
	sacc_removeTarWait:
		do timer_set1_shell(0,0,0,0,200,0)
		to sacc_removeTar on +MET % timer_check1
	sacc_removeTar:
		do drawSaccTarg(1, 0)
		to sacc_removeTarCd on MAT_WENT % drawTarg_done
	sacc_removeTarCd:
		do ec_send_code(TARGC1OFFCD)
		to sacc_waitfpoff
	sacc_waitfpoff:
		do timer_set1_shell(1000,1000,2000,400,0,0)
		to sacc_fpoff_branch on +MET % timer_check1
	sacc_fpoff_branch:
		to sacc_fpoff_memory on 1 = gl_saccdelay /* branch to memory */
		to sacc_fpoff
	sacc_fpoff:
		do drawSaccTarg(0,1)
		to sacc_fpoff_cd on MAT_WENT % drawTarg_done
	sacc_fpoff_memory:
		do drawSaccTarg(0,0)
		to sacc_fpoff_cd on MAT_WENT % drawTarg_done
	sacc_fpoff_cd:
	...
	sacc_grace:
		time 2000
		to sacc_saccmd on +WD0_XY & eyeflag
		to sacc_end
	sacc_saccmd:
		do ec_send_code(SACMADCD)
		to sacc_check_response
	sacc_check_response:
		time 50
		to sacc_tarhold on -WD3_XY & eyeflag /* got it!	*/
		to sacc_end
	sacc_tarhold:
		do ec_send_code(TRGACQUIRECD)
		time 50   /* gotta hold for this long	*/
		to sacc_end on +WD3_XY & eyeflag /* didn't hold */
		to sacc_rew
	sacc_rew:
		do give_reward(REW, 150, CORRECT)
		to sacc_rew_off on +MET % timer_check1
	sacc_rew_off:
		do dio_off(REW)
		to sacc_end
	sacc_end:
	...
	*/
    ...
    time 100 /* delay before activating eye_flag - noise on the eye position signal should not be able to "break eye fixation" */
		do set_fix_rwd()
		to fixreward on 1 = gl_rew_fix
		to fixeyeset
	fixreward:
		do give_prize(REW, 50, PRIZE)
		to fixrewardoff  on +MET % timer_check1
	fixrewardoff:
		do dio_off(REW)
		to fixeyeset
...
***/
	teye_start:
	  do setup_eyewindows(25, 25)
		to teye_fp
  teye_fixed: /* fixed target windows */
    do setup_hemiwindows(60, 0, 60, 70)
    to teye_fp
	teye_fp:
		do drawTarg(1000,0,0,0,0)
  ...
  teye_targ_def:
      ...
      to teye_targ_def0 on 1 < gl_rt_flag /* Branch to ASYNCH task if RT flag>1 */
      to teye_targ
	teye_targ_def0: /* For asynchronous targets */
			do defTargBlkWhite()
			to teye_targ0
	teye_targ0: /* show the targets */
			do drawTarg(1000, 1000, 1000, -1, 0)
			to teye_targ_cd0 on MAT_WENT % drawTarg_done
	teye_targ_cd0:
			do ec_send_code(TARGONCD)
			to teye_async_timer0
	teye_async_timer0:
			do set_async_timer(1)
			to teye_targ_def1 on +MET % timer_check1
	teye_targ_def1:
			do defTargLum(-1, 1000, 1000, -1)
			to teye_targ1
	teye_targ1: /* show both targets */
			do drawTarg(1000, 1000, 1000, -1, 0)
			to teye_targ_cd1 on MAT_WENT % drawTarg_done
	teye_targ_cd1:
			do ec_send_code(TARGONCD)
			to teye_async_timer1
	teye_async_timer1:
			do set_async_timer(2)
  ...
  teye_targ: /* show the targets */
	...
	teye_targ_cd:
			do ec_send_code(TARGC1CD)
      to fc_tarwait on 1 < gl_rt_flag /* Branch to ASYNCH task if RT flag>1 */
			to teye_waitdots
    ...
    fc_saccd:
        do ec_send_code(SACMADCD)
        to fc_check_eye_response on 2 = gl_rt_flag /* Free choice ignoring target asynch */
        to check_eye_response /* Rewards choosing earlier presented target */
    fc_check_eye_response:
        time 50				 /* checking windows takes time! around 20 ms per window seems ok, 25 to be safe */
        to fc_ceyehold on -WD3_XY & eyeflag		/* got correct target as per asynchrony*/
        to fc_neyehold on -WD4_XY & eyeflag		/* got incorrect target! */
        to ncshow                               /* got nada RECHECK NC CONDITION */
    fc_ceyehold:
        do ec_send_code(TRGACQUIRECD)
        time 200	/* gotta hold for this long	*/
        to ncshow on +WD3_XY & eyeflag /* RECHECK NO HOLD CONDITION */
        to fc_cchoice
    fc_neyehold:
        do ec_send_code(TRGACQUIRECD)
        time 200 /* gotta hold for this long	*/
        to ncshow on +WD4_XY & eyeflag /* RECHECK NO HOLD CONDITION */
        to fc_nchoice
    fc_cchoice:
        do total(CORRECT) /*code is dropped within the function*/
        to fc_reward
    fc_nchoice:
        do total(WRONG) /*code is dropped within the function*/
        to fc_reward
    fc_reward:
        do prob_reward()
        to prrew on 1 = gl_rew_fc /* reward when flag set to 1 */
        to nuend /* no reward on other trials */
