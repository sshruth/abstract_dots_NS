...
int                             gl_color_flag;
int                             gl_color_R; // only used if gl_color_flag is 0
int                             gl_color_G; // only used if gl_color_flag is 0
int                             gl_color_B; // only used if gl_color_flag is 0
...
//		gl_targObj.y = storey;
  ...
  printf("Trials remaining: %d \n", gl_trialCtr);
	...
	printf("gl_color_flag: %d \n", gl_color_flag);
	if (gl_color_flag == 0) { // take from RGB values entered in menu
		if (gl_color_R == 0 && gl_color_G == 0 && gl_color_B == 0) { // if black on black background
			printf("WARNING: Trying to display black target on black background!\n");
			printf("WARNING: Showing white target instead.\n");
			gl_targObj.color.R = 255;
			gl_targObj.color.G = 255;
			gl_targObj.color.B = 255;
		} else {
			gl_targObj.color.R = gl_color_R;
			gl_targObj.color.G = gl_color_G;
			gl_targObj.color.B = gl_color_B;
		}
	} else if (gl_color_flag == 1) { // white
		gl_targObj.color.R = 255;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 255;
	} else if (gl_color_flag == 2) { // my Yellow (SS)
		gl_targObj.color.R = 212;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 0;
	} else if (gl_color_flag == 3) { // my Cyan (SS)
		gl_targObj.color.R = 0;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 255;
	} else if (gl_color_flag == 4) { // Red
		gl_targObj.color.R = 255;
		gl_targObj.color.G = 0;
		gl_targObj.color.B = 0;
	} else if (gl_color_flag == 5) { // Green
		gl_targObj.color.R = 0;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 0;
	} else if (gl_color_flag == 6) { // Blue
		gl_targObj.color.R = 0;
		gl_targObj.color.G = 0;
		gl_targObj.color.B = 255;
	} else if (gl_color_flag == 7) { // Yellow
		gl_targObj.color.R = 255;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 0;
	} else if (gl_color_flag == 8) { // Cyan
		gl_targObj.color.R = 0;
		gl_targObj.color.G = 255;
		gl_targObj.color.B = 255;
	} else if (gl_color_flag == 9) { // Magenta
		gl_targObj.color.R = 255;
		gl_targObj.color.G = 0;
		gl_targObj.color.B = 255;
	}
	printf("Target [R G B]: [%d  %d  %d] \n", gl_targObj.color.R, gl_targObj.color.G, gl_targObj.color.B);
	// printf("Target R: %d \n", gl_targObj.color.R);
	// printf("Target G: %d \n", gl_targObj.color.G);
	// printf("Target B: %d \n", gl_targObj.color.B);
...
VLIST state_vl[] = {
  ...
  {"RewardFixation", 	          &(gl_reward_fixation), 	NP, NP, 0, ME_DEC},
	{"Color_WmYmC_RGB_YCM", 			&(gl_color_flag), 		  NP, NP, 0, ME_DEC},
	{"TargetR", 									&(gl_color_R), 		  		NP, NP, 0, ME_DEC},
	{"TargetG", 									&(gl_color_G), 		  		NP, NP, 0, ME_DEC},
	{"TargetB", 									&(gl_color_B), 		  		NP, NP, 0, ME_DEC},
	{NS}};
