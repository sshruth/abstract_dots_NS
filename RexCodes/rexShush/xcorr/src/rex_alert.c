...
puts("-alert: before return***\n");
		...
		p->p_msg &= ~s_(G_KILL);
		release_(&p->p_sem);
		p->p_state |= P_EXIT_ST;	/* let comm know that I'm terminating */
		PtExit(0);
	}
		...
		p->p_msg &= ~s_(G_RUN);
		release_(&p->p_sem);
		if(i_b->ph_xcorr_pi > 0) {
			rxerr("alert(): A ph_ras process is already running");
			r_sendmsg((PROCTBL_P)&i_b->ptbl[i_b->ph_switch_pi], CM_NEG);
			goto newscan;
		}
		i_b->ph_xcorr_pi= myptx;
...
oncntrlb(int sig)
{
	...
	doutput_filename= "/tmp/doutput_ph_ras";
