...
#undef  AD_BASE 0x240	/* ISA a/d base address */ 
#undef  AD_VECT 10	/* ISA a/d interrupt vector */ 
...
#undef  PCDIO		/* Kontron PCDIO120 ISA digital i/o */
#undef  KMPIO12		/* Keithly-Metrabyte digitial i/o */
#undef  PRIM_ISA_DIO_BASE 0x280	/* base address of primary ISA digital i/o */

/*
 * Misc defines.
 */
#undef DIO_TRUE_LOW	     /* #define if a low voltage on digital input should be interpreted as a 1
...
#define LATCH_UNITS	    /* single units on latch inputs */ 
#undef  MEXWEX_UNITS	/* receive MEX or WEX units */
#undef  MEX62_UNITS		/* receive new mex units */
#undef  MEXWEX_PORT 3   /* digital i/o port on PCDIO card used for MEX/WEX 
						 * communication, range 2-4 */
#undef  MEX62_PORT 3	/* digital i/o port on PCDIO card used for MOMENTICS MEX
						 * communication, range 2-4 */
#undef  TC_UNITS	    /* units acquired with 9513 timer chips from Plexon box */ 
#undef  TC_NUM 8	    /* number of 9513 chips in use.  Each chip can input 5 unit channels,
						 * there are 4 chips per card, max 2 cards possible */
#undef  TC_CARD0_BASE 0x340 /* base address of counter timer card 0; note PCI version is not
							 * detected plug and play- you must put it's base address here */
#undef  TC_CARD1_BASE 0x348 /* base address of counter timer card 1 */
...
#undef  PCMESSAGE_PORT 4/* digital i/o port on PCDIO card used for PC Messaging, 
						 * range is from 2-4 */
#undef  PCS_SOCKET	    /* PC to PC messaging code with sockets */ 
#undef  HARDWARE_SWITCH	/* enable hardware PSTOP and RLSTOP switches */ 
#undef  SMI_TRACK	    /* SMI Eye tracker */ 
#define PLEX_ECODES  	/* REX sends ecodes to Plexon box */
#define PLEX_PORT 3     /* which digital i/o port on the PCDIO board connected to the
						 * Plexon box, range is from 2-4 */

#define PLEX_POLARIS         /* Rex sends Polaris spectra coordinates to Plexon box */
#define PLEX_POLARIS_PORT  4 /* which digital i/o port
...
#define HWD_MAXNUM 5    /* max number of hand monitoring (Polaris) windows, don't make this > 5 */
...
#define pl_send_code_(code)  { \
     	out8(PL_DIOBASE+2, in8(PL_DIOBASE+2) & ~PL_STROBE); /* clear strobe */  \
		/*out8(PL_DIOBASE + 2, 0);*/ \
        out8(PL_DIOBASE, code & 0xff); \
		/*idprintf("\nPL_DIOBASE sent = %d",code & 0xff);*/ \
		out8(PL_DIOBASE+1, (code >> 8)); \
		/*idprintf("\nPL_high sent = %d",(code >> 8));*/ \
		/*out8(PL_DIOBASE + 2, PL_STROBE);*/ \
	    out8(PL_DIOBASE+2, in8(PL_DIOBASE+2) | PL_STROBE); /* set strobe */ \ 
	}

#define pl_start_record_() { \
	out8(PL_DIOBASE+2, in8(PL_DIOBASE+2) | PL_RECORD); \
  }

#define pl_stop_record_() { \
	out8(PL_DIOBASE+2, in8(PL_DIOBASE+2) & ~PL_RECORD); \
  }
#endif
 ...
 * 
 * 
 * If we're recording arm position with the Polaris Spectra system,
 * we'll use the last port on the PC_DIO to send the data to Plexon.
 * This will make it much easier to synchronize and store everything
 * In turn, Plexon, if it's already receiving ecodes, will require
 * an additional digital in sub-board on the DSP mainboard.
 * 
 * 
 * Luke
 * 
...
#ifdef PLEX_POLARIS
#define PLPOL_DIOBASE   ((i_b->i_prim_dio_base)+(PLEX_POLARIS_PORT*4))  /* base address of 8255 connected to Plexon box (second di board) */
#define PLPOL_STROBE         0x1	                                    /* Plexon strobe bit */
#define PLPOL_BUSY	         0x10                                       /* Plexon busy bit */
#define	PLPOL_MAXPOSITIONS   4*25 //frameN, x, y, z	                    /* max number of frames buffered */
#define plpol_box_ready_ (!(in8(PLPOL_DIOBASE+2) & PLPOL_BUSY))
 ...
 * give time for cable deskew, and 8255 settling.
 */
#define plpol_send_position_(position)  { \
		out8(PLPOL_DIOBASE+2, 0); /* clear strobe */  \
		out8(PLPOL_DIOBASE, position & 0xff); \
	    /* idprintf("\nPLPOL_DIOBASE sent = %d",position & 0xff); */ \
		out8(PLPOL_DIOBASE+1, (position >> 8)); \
 	    /* idprintf("\nPLPOL_high sent = %d",(position >> 8)); */ \
		out8(PLPOL_DIOBASE+2, PLPOL_STROBE); /* set strobe */ \
	}
#endif
...
/* Shadlen lab version: */
#define LATCH1 Dio_id(PCDIO_DIO,10,0x01)
#define LATCH2 Dio_id(PCDIO_DIO,10,0x02)
#define LATCH3 Dio_id(PCDIO_DIO,10,0x04)
#define LATCH4 Dio_id(PCDIO_DIO,10,0x08)
#define LATCH5 Dio_id(PCDIO_DIO,10,0x10)
#define LATCH6 Dio_id(PCDIO_DIO,10,0x20)
#define LATCH7 Dio_id(PCDIO_DIO,10,0x40)
#define LATCH8 Dio_id(PCDIO_DIO,10,0x80)
...
/* Shadlen lab: */
#define DIN_UNITS	Dio_id(PCDIO_DIO, 8, 0xff)  /* 8 unit channels */
