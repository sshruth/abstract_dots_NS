...
/*
	cbreak_tty: Attempts to open a tty device by pathname (ie.
	/dev/ttyi2), and then will set terminal up for cbreak i/o
	mode.
...
int	tty_fd;
	...
	cur_stty = org_stty;		/* copy information	*/
	cur_stty.sg_flags |= CBREAK;	/* raw mode (all 8 bits pass)	*/
/*	cur_stty.sg_flags &= ~(TANDEM|ECHO|CRMOD); remove all char processing */
