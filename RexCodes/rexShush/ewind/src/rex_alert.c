...
puts("-alert: before return***\n");
		...
		/* clear the D_WIND bit in d_flags */
		i_b->d_flags &= ~D_WIND;
		...
		p->p_msg &= ~s_(G_RUN);
		release_(&p->p_sem);
		if(i_b->ph_ewind_pi > 0) {
			rxerr("alert(): A ph_ewind process is already running");
			r_sendmsg((PROCTBL_P)&i_b->ptbl[i_b->ph_switch_pi], CM_NEG);
			goto newscan;
		}
		i_b->ph_ewind_pi = myptx;
...
oncntrlb(int sig)
{
	...
	doutput_filename= "/tmp/doutput_ph_ewind";
