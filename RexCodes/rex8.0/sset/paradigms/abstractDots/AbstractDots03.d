...
//static int gl_coh_list[] = {32, 64, 91, 128, 181, 256, 362, 512, 756};
...
//	}
    ...
    if (TOY_RAND(1000) < 500) {
        gl_vsd->display->object_array[i]->vertex = gl_vsd->display->object_array[i]->x;
        gl_vsd->display->object_array[i]->wrt = gl_vsd->display->object_array[i]->y;
        gl_vsd->display->object_array[j]->vertex = gl_vsd->display->object_array[j]->x;
        gl_vsd->display->object_array[j]->wrt = gl_vsd->display->object_array[j]->y;
        
    } else { /* flip about the horizontal axis */
        gl_vsd->display->object_array[i]->vertex = gl_vsd->display->object_array[i]->x;
        gl_vsd->display->object_array[i]->wrt = -1 * gl_vsd->display->object_array[i]->y;
        gl_vsd->display->object_array[j]->vertex = gl_vsd->display->object_array[j]->x;
        gl_vsd->display->object_array[j]->wrt = -1 * gl_vsd->display->object_array[j]->y;
...
VLIST state_vl[] = {
	...
	{"Number of targs", 		&(gl_task_infoS[0].proportion), 	NP, NP, 0, ME_DEC},
