...
/* ROUTINE sendWindowsToPolaris
 * 
 * sends to Polaris the center and half extent of the 
 * two hand response windows, remember these have been
 * multiplied by 100 to get rid of decimals. 1 indicates
 * that we'll send the position of the response cubes, 0 indicates
 * that we'll "erase them"
 * 
 */

int sendWindowsToPolaris(int flag, long halfextentx, long halfextenty, long halfextentz)
{
	_VSobject vso;
	char sndBuf[256];
	int 
...
//	cm_per_deg = 1./deg_per_cm;
	...
	if (flag) 
	  sprintf(sndBuf, "%d %d %d %d %d %d %d %d %d",
			  centerx1, centery1, centerz1,
			  centerx2, centery2, centerz2,
			  halfextentz, halfextentx, halfextenty);
	else 
	  sprintf(sndBuf, "%d %d %d %d %d %d %d %d %d",
			  0, 0, 0, 0, 0, 0, 0, 0, 0);
	udp_send(polarisIP, sndBuf);
...
begin	first:
	...
	fixeyehandwait:    /* wait for either eye or hand fixation */
	    ...
	    to fixhand1delay on -HWD0_XYZ & handflag
		to nofix
	...
	fixeye1set:		/* set flag to check for eye fixation breaks */
	    ...
	    to fixhand2delay on -HWD0_XYZ & handflag
	    to nofix
	fixhand2delay:
	...
	fixeyehanddone1:		
	    do position_eyewindow(25, 25, 6)
	    to fixeyehanddone2
	fixeyehanddone2:		/* put hand fix window at current hand position */
	                        /**** also TASK BRANCHING ... ****/
	    do position_handwindow(50, 50, 50, 6)
...
***/
	...
	thand_rtdotdur:	/* set the dots timeout */
		do set_dots_duration(0,0,0,0,5000,0)
		to thand_rtsacc on +HWD0_XYZ & handflag	/* hand movement detected */
	...
	thand_setdotdur:		/* set the dots duration timer */
		do set_dots_duration(1000,100,1500,300,0,0)
		to thand_stopdots on +MET % timer_check1 /* check the timer */
	thand_stopdots:
		do stopDots()
	...
	*/
	...
	thand_gracea:
		time 500
		to thand_moved on +HWD0_XYZ & handflag
		to ncshow
	thand_moved:
		do ec_send_code(REACHMADCD)
		to check_hand_response
