...
int 														gl_rew_fix_prob = 100; /*probability of being rewarded for fixation*/
 ...
 **  Sets the sequence of target appearances
 */
int defTargBlkWhite()
{
    RGB color;
    ...
    int asyncf[5] = {20, 12, 9, 6, 3};
    int async_max = 21; // Set to one more than highest async used
    _VSDdot_object dot = VSD_GET_DTOBJECT(gl_vsd);
    coh = dot->coherence;
    angl = dot->direction;

		if (gl_async_ctr==0){ /* Assign async according to trial coherence */
        if (coh==512){
    ...
    obj_v = VSD_GET_T1(gl_vsd);
    color.R = tar1val;
    color.G = 0;
    color.B = 0;
    mat_targDefine(2, obj_v->x, obj_v->y, obj_v->diameter, &color);

    obj_v = VSD_GET_T2(gl_vsd);
    color.R = tar2val;
    color.G = 0;
    color.B = 0;
    mat_targDefine(3, obj_v->x, obj_v->y, obj_v->diameter, &color);

    if (gl_async_ctr<async_max)
    gl_async_ctr++;
    else gl_async_ctr = 0;
    return 0;
}
...
//	   	printf("\n");
	...
	/* save the parameters */
	/* task identifier */
    EC_TAG2(I_TRIALIDCD,30+gl_rt_flag); // 30 is variable duration dots, 31 is RT, 32 is freechoice, 33 is async target
...
***/
	...
	teye_targ_def0: /* For asynchronous targets */
		do defTargBlkWhite()
		to teye_targ_cd on 0 = gl_async_ctr
        to teye_targ0
