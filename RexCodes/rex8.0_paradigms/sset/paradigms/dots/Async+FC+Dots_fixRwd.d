...
int                             gl_async_max = 8; /* asynchrony maximum frames */
...
int defTargBlkWhite()
{
    RGB color;
    _VSobject obj_v;
    int tar1val, tar2val, angl, lateLum;
    _VSDtrial_info ctrial_info;
    ctrial_info = gl_vsd->cur_trial->trial_info;
    angl = ctrial_info->planet_angle;

    if (gl_async_ctr==0){ /* Pick a random number if start of counter */
        gl_async_cur = TOY_RAND(gl_async_max);
...
VLIST state_vl[] = {
	...
	{"Max Async",               &gl_async_max,                      NP, NP, 0, ME_DEC},
