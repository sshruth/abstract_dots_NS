...
//	}
    ...
    if (gl_rt_flag >= 0){ /* For variable duration (0), RT (1), and free choice (2). Note that this change may screw up the functionality to pick target locations based on dot direction/angle. */
			...
			if (TOY_RAND(1000) < 500){
					j = gl_menu_infoS.rfy1 + TOY_RAND(gl_menu_infoS.rf_jit);
			} else {
					j = gl_menu_infoS.rfy1 - TOY_RAND(gl_menu_infoS.rf_jit);
			}
			gl_vsd->display->object_array[VSD_OBJECT_T1]->x = i;
			gl_vsd->display->object_array[VSD_OBJECT_T1]->y = j;

			if (TOY_RAND(1000) < 500){
					i = gl_menu_infoS.rfx2 + TOY_RAND(gl_menu_infoS.rf_jit);
			} else {
					i = gl_menu_infoS.rfx2 - TOY_RAND(gl_menu_infoS.rf_jit);
			}
			if (TOY_RAND(1000) < 500){
					j = gl_menu_infoS.rfy2 + TOY_RAND(gl_menu_infoS.rf_jit);
			} else {
					j = gl_menu_infoS.rfy2 - TOY_RAND(gl_menu_infoS.rf_jit);
			}
			gl_vsd->display->object_array[VSD_OBJECT_T2]->x = i;
			gl_vsd->display->object_array[VSD_OBJECT_T2]->y = j;
    }
...
VLIST state_vl[] = {
    {"Repetitions",			    &(gl_menu_infoS.repetitions),		NP, make_tasks, ME_AFT, ME_DEC},
		{"RF1_x",                   &(gl_menu_infoS.rfx1),		        NP, NP, 0, ME_DEC},
    {"RF1_y",                   &(gl_menu_infoS.rfy1),		        NP, NP, 0, ME_DEC},
    {"RF2_x",                   &(gl_menu_infoS.rfx2),	    	    NP, NP, 0, ME_DEC},
    {"RF2_y",                   &(gl_menu_infoS.rfy2),  		    NP, NP, 0, ME_DEC},
    {"RF_jit",                  &(gl_menu_infoS.rf_jit),            NP, NP, 0, ME_DEC},
